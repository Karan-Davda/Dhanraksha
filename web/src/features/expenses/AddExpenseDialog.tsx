...
import type { Currency, ExpenseCategory, PaymentMethod } from '../../types/domain'
...
export type AddExpenseValues = z.infer<typeof Schema>
...
export function AddExpenseDialog(props: {
  ...
  const defaultValues = useMemo<AddExpenseValues>(
    () => ({
      amount: 0,
  ...
  const handleSubmit: SubmitHandler<AddExpenseValues> = async (values) => {
            ...
            <Controller
              ...
              render={({ field }) => (
                <TextField
                  ...
                  fullWidth
                  {...field}
                  error={Boolean(form.formState.errors.payment_method)}
                  helperText={form.formState.errors.payment_method?.message}
                >
